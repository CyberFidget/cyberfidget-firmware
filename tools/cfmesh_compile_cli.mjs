...
const fallbackCompiler = path.resolve(
  repoRoot, '..', 'cyberfidget_website', 'assets', 'js', 'cfmesh_compile.mjs',
);

function usage() {
  return 'Usage: node tools/cfmesh_compile_cli.mjs [--check-only] [--symbol-prefix NAME] INPUT OUTPUT';
}
  ...
  if (args.checkOnly) {
    ...
    if (existing !== header) {
      throw new Error(
        ...
        + 'Regenerate target: pio run -t cfmesh-regen',
      );
    }
