...
def read_version_txt(repo_root):
...
def select_build_type(git_available, dirty, override):
    if override:
        return override
...
def build_full_string(major, minor, patch, git_hash, dirty):
...
def render_version_h(major, minor, patch, git_hash, dirty,
...
def write_atomic_if_changed(out_path, content):
...
def build_hints(build_type, in_ci):
...
def print_summary(full, build_type, build_timestamp, output_artifact, hints):
    out = sys.stdout
    out.write("\n")
    for hint in hints:
        out.write(hint + "\n")
...
def generate(repo_root, out_path, output_artifact=None, build_type_override=None):
