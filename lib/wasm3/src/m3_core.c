...
// CYBERFIDGET SPIKE PATCH (spike/wasm3-apps): when d_m3CustomAllocator is
// defined, the default m3_Malloc_Impl/m3_Realloc_Impl/m3_Free_Impl below are
// compiled out and the host must provide them (see lib/WasmAppRuntime, which
// routes allocations to ESP32 PSRAM via heap_caps_*). Upstream wasm3 has no
// allocator hook; this is the only local modification to the vendored source.
#if defined(d_m3CustomAllocator)

// Host provides m3_Malloc_Impl / m3_Realloc_Impl / m3_Free_Impl.
