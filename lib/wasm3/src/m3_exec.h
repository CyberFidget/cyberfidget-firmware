...
d_m3Op  (Entry)
{
    ...
    if (true)
#else
    // CYBERFIDGET SPIKE PATCH (spike/wasm3-apps): also trap when the host
    // reports native (C) stack exhaustion — wasm calls recurse natively and
    // upstream has no guard (overrun = hard reset instead of a trap).
    if (M3_LIKELY ((void *) (_sp + function->maxStackSlots) < _mem->maxStack
#   if defined(d_m3NativeStackGuard)
                   and not m3_NativeStackExhausted ()
#   endif
                  ))
#endif
    {
